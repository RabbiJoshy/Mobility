...
day = pd.DataFrame()
for i in os.listdir(os.path.join('Bike 0331')):
    if i != '.DS_Store':
    # print(i)
        NSTest = NS(os.path.join('Bike 0331', i))
        NSTest = NSTest[['station_id', 'num_bikes_available','time', 'name', 'lat', 'lon']]
        day = pd.concat([day, NSTest])

daysorted = day.sort_values(by = ['station_id', 'time'])

daysorted.to_pickle('NS3103sorted')